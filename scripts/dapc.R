...
genl_females <- genl[result$Sex == "F"]

genl <- genl_females
allele_freq <- glMean(genl)

# keep loci that are polymorphic 
polymorphic_loci <- locus_var > 0 & !is.na(locus_var)
genl_filtered <- genl[, polymorphic_loci]
genl <- genl_filtered

## Full data set
pca1 <- glPca(genl,center = T, scale = T, nf = 5)

#png("../figures/dapc_eigen_females.png", h=4, w=4, units="in", res=300)
barplot(100*pca1$eig/sum(pca1$eig), col = heat.colors(50), main="PCA Eigenvalues") # retain first 5 axes, incremental decrease after 2
title(ylab="Percent of variance\nexplained", line = 2)
title(xlab="Eigenvalues", line = 1)
#dev.off()
...
ggsave("../figures/dapc_PCA_females.png",pca1.p, h=5, w=5)
...
png(file="../figures/dapc.females.png", h=4, w=4, units="in", res=300)
scatter(dapc1, grp=genl@pop, 
        bg="white", pch=c(15,19,17,18), col=col.in,
        cex=2,
        cstar=0,clab=0,
...
png(file="../figures/dapc.2.females.png", h=4, w=4, units="in", res=300)
scatter(dapc2, grp=genl2@pop, posi.da="bottomright", 
...
# leave out ~30% of samples. 42 indivs. so select 29 to keep (drop 13)
kept.id <- sample(1:nInd(genl), size=29, replace=F)
...
png(filename = "../figures/dapc.4pcs.accuracyPops_females.png", h=6, w=6, units="in", res=300)
boxplot(df.out.4, ylab="Proportion correct assignment",
        col=c("#eac435","#557fc3", "#03cea4", "#fb4d3d"))
dev.off()
...
for(i in 1:n){
  kept.id <- sample(1:nInd(genl2), size=29, replace=F)
  x <- genl2[kept.id,]
  x.sup <- genl2[!1:nInd(genl2) %in% kept.id,]
  # run dapc
  dapc2 <- dapc(x, x@pop, n.pca=1, n.da=2)
...
for(i in 1:n){
  ...
  x.sup <- genl[!1:nInd(genl) %in% kept.id,]
  # run dapc
  dapc4 <- dapc(x, x@pop,n.pca=10,n.da=3)
  pred.sup <- predict.dapc(dapc4, newdata=x.sup )
  
  # calculate accuracy of assignments. i.e., is anything coming from its actual population
  acc.out.4.10pcs[i] <- mean(as.character(pred.sup$assign)==as.character(pop(x.sup)))
  if(i%%50 ==0){print(i)}
}

boxplot(acc.out.4.10pcs)
...
library("poppr")
pramx <- xvalDapc(tab(genl2, NA.method = "mean"), pop(genl2)) # 15 is best
...
png(file="../figures/dapc.2.15pcs_females.png", h=4, w=4, units="in", res=300)
scatter(dapc2, grp=genl2@pop, 
        bg="white", pch=c(15,19,17,18), col=col.in,
        cex=2,
        cstar=0,clab=0,
...
for(i in 1:n){
  ...
  # calculate accuracy of assignments. i.e., is anything coming from its actual population
  acc.out.2.15pcs[i] <- mean(as.character(pred.sup$assign)==as.character(pop(x.sup)))
  if(i%%50 ==0){print(i)}
}

boxplot(acc.out.2.15pcs)
...
dplot <- data.frame(k = c(rep("4pops.3pcs", length(acc.out.4)),
                          rep("2pops.1pcs", length(acc.out.2)),
                          rep("4pops.10pcs", length(acc.out.4.10pcs)),
                          rep("2pops.15pcs", length(acc.out.2.15pcs))
),
accuracy = c(acc.out.4,
             acc.out.2,
             acc.out.4.10pcs,
             acc.out.2.15pcs)
)
dplot$k <- factor(dplot$k, levels=c("2pops.1pcs", "2pops.15pcs", "4pops.3pcs", "4pops.10pcs"))
library(ggplot2)
library(ggbeeswarm)
...
ggsave(file="../figures/dapc_accuracy_females.pdf", p, h=5, w=5)
ggsave(file="../figures/dapc_accuracy_females.png", p, h=5, w=5)
...
# 2pops.1pcs       0.514 0.133  0.0133 
# 2pops.15pcs      0.568 0.117  0.0117 
# 4pops.3pcs       0.236 0.0903 0.00903
# 4pops.10pcs      0.268 0.119  0.0119 



write.csv(file="dapc_training_test_females.csv",dplot, row.names=F)
