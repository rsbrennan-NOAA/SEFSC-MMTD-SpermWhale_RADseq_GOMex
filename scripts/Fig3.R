...
dat <- read.table("analysis/variants_NoLD_unrelated_femalesOnly_PCA.eigenvec", header=F)
eigenval <- read.table("analysis/variants_NoLD_unrelated_femalesOnly_PCA.eigenval", header=F)
...
ggsave("../figures/pca_females.png", p_pca)
...
fdat <- subset_snpR_data(dat,.facets="sex", .subfacets = "F")
fdat <- filter_snps(fdat, maf=0.05)
# bi-allelic snpRdata with 4226 SNPs and 48 samples.
fdat

# calculate fst between the populations
my.dat <- calc_pairwise_fst(fdat, facets="pop", method = "WC", boot = 500)


# the bootstrapping here is actually permutation, mixing pop assignments
fst_pvals <- get.snpR.stats(my.dat, facets = "pop", stats = "fst")$fst.matrix$pop$p

get.snpR.stats(my.dat, facets = "pop", stats = "fst")$fst.matrix$pop

#$fst
#Key: <p1>
#  p1  Dry_Tortuga      NGOMex      WGOMex
#<char>        <num>       <num>       <num>
#  1:    Atlantic 8.120421e-05 0.008595930 0.006902738
#2: Dry_Tortuga           NA 0.007378556 0.006555871
#3:      NGOMex           NA          NA 0.009811148

#$p
#Key: <p1>
#  p1 Dry_Tortuga    NGOMex    WGOMex
#<char>       <num>     <num>     <num>
#  1:    Atlantic   0.4431138 0.2075848 0.2175649
#2: Dry_Tortuga          NA 0.2355289 0.2295409
#3:      NGOMex          NA        NA 0.1137725
# convert p-values to long:
...
# save p-values
write.csv(melt_fst_pval, file="fst_pvalues_females.csv", quote=F, row.names = F)
...
# save fst
write.csv(melt_fst, file="fst_females.csv", quote=F, row.names = F)
...
fst_plot_dat3$value2 <- as.character(round(fst_plot_dat3$value, 3))
fst_plot_dat3$value2[1] <- "0.0001"

p_fst <- ggplot(fst_plot_dat3, aes(y = Var2, x = Var1, fill = value)) +
  geom_tile(size=0.2, color="black") +
  geom_text(aes(label = value2), na.rm = TRUE, size=3) +
  scale_fill_gradient(limits = range(fst_plot_dat3$value, na.rm = TRUE),
                      low = "#FFCCCC", 
                      high = "firebrick3", 
                      na.value = "white") +  
...
dat_dapc <- read.csv("dapc_training_test_females.csv")
...
p_out1 <- ggarrange(p_fst2, p_dapc2, ncol = 1, nrow = 2, labels=c("B", "C"))
p_out <- ggarrange(p_pca,p_out1, ncol=2, widths = c(1, 0.75), labels=c("A", "", ""))
ggsave(file="figures/fig3_females.pdf", w=7, h=4)
ggsave(file="figures/fig3_females.png", w=7, h=4)
...
library(snpR)
dat <- snpR::read_vcf("analysis/filtered.final_newMAF.vcf.gz")
pops <- read.csv("SW_Metadata.csv")
...
#p1 Dry_Tortuga    NGOMex     WGOMex
#<char>       <num>     <num>      <num>
#1:    Atlantic   0.4251497 0.1776447 0.13173653
#2: Dry_Tortuga          NA 0.1696607 0.09980040
#3:      NGOMex          NA        NA 0.06187625
...
# save p-values
write.csv(melt_fst_pval, file="fst_pvalues_maf01.csv", quote=F, row.names = F)
...
# save fst
write.csv(melt_fst, file="fst_maf01.csv", quote=F, row.names = F)
...
ggsave(file="figures/fst_maf01.png", p_fst, h=3, w=3)
